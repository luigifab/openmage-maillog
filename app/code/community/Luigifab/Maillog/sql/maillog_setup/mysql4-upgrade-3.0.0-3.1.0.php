...
try {
	...
	if ((mb_stripos($sql, 'MariaDB') !== false) && version_compare($sql, '10.0.2', '>=')) {
		$this->run('ALTER TABLE '.$this->getTable('maillog/email').' ADD COLUMN IF NOT EXISTS mail_sender varchar(255) NOT NULL DEFAULT ""');
		$this->run('ALTER TABLE '.$this->getTable('maillog/email').' ADD COLUMN IF NOT EXISTS deleted tinyint(1) unsigned NOT NULL DEFAULT 0');
	}
	else {
		$this->run('ALTER TABLE '.$this->getTable('maillog/email').' ADD COLUMN mail_sender varchar(255) NOT NULL DEFAULT ""');
		$this->run('ALTER TABLE '.$this->getTable('maillog/email').' ADD COLUMN deleted tinyint(1) unsigned NOT NULL DEFAULT 0');
	}

	// rend nullable les colonnes
	$this->run('ALTER TABLE '.$this->getTable('maillog/email').' MODIFY COLUMN encoded_mail_recipients varchar(255) NULL DEFAULT NULL');
	$this->run('ALTER TABLE '.$this->getTable('maillog/email').' MODIFY COLUMN encoded_mail_subject    varchar(255) NULL DEFAULT NULL');
	$this->run('ALTER TABLE '.$this->getTable('maillog/email').' MODIFY COLUMN mail_recipients         varchar(255) NULL DEFAULT NULL');
	$this->run('ALTER TABLE '.$this->getTable('maillog/email').' MODIFY COLUMN mail_subject            varchar(255) NULL DEFAULT NULL');
	$this->run('ALTER TABLE '.$this->getTable('maillog/email').' MODIFY COLUMN mail_body               longtext     NULL DEFAULT NULL');
	$this->run('ALTER TABLE '.$this->getTable('maillog/email').' MODIFY COLUMN mail_header             text         NULL DEFAULT NULL');
	$this->run('ALTER TABLE '.$this->getTable('maillog/email').' MODIFY COLUMN mail_parameters         text         NULL DEFAULT NULL');
	$this->run('ALTER TABLE '.$this->getTable('maillog/email').' MODIFY COLUMN mail_parts              longblob     NULL DEFAULT NULL');
